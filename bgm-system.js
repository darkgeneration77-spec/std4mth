...
function schedule(){if(!ctx||!enabled)return;const tr=TRACKS[current],beat=60/tr.bpm/2;while(nextTime<ctx.currentTime+.8){const i=step%16,n=tr.lead[i],b=tr.bass[Math.floor(i/2)%tr.bass.length];tone(midi(n),nextTime,beat*.82,tr.wave,current==='boss'?.09:.07);if(i%2===0)tone(midi(b),nextTime,beat*1.75,'sine',current==='boss'?.11:.08);if(i%4===0)kick(nextTime,current==='boss'?.16:.11);hat(nextTime+beat*.5,current==='boss'?.045:.025);nextTime+=beat;step++}}
